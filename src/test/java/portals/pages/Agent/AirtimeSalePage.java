...
public class AirtimeSalePage extends SetupInit {
	MobileTopupPageOperations mobileTopupOperations;
	DashBoardPageOperations dashboardPageCommon;
	CommonOperations common;
	elasticwrite log;

	public AirtimeSalePage(WebDriver driver, elasticwrite log) {
		this.log = log;
		this.driver = driver;
	...
	public Map<Object, Object> airtimeSale(Map<Object, Object> map) {
			...
			if (verifyVisible(dashboardPageCommon.alert, 2)) {
				...
				if (!map.get(MobileNumber).toString().isEmpty())
					verifyVisible(By.xpath(
							...
							0);
				// else
				// verifyVisible(By.xpath(String.format(dashboardPageCommon.verifyMobileNumber,
				// userName)), 0);
